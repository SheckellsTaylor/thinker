...
import type { GeneratedProject, Project } from '@/lib/types'
...
export default function Home() {
  ...
  async function handleGenerate(idea: string) {
    ...
    try {
      const res = await fetch('/api/grow-idea', {
        method: 'POST',
  ...
  async function handleSelectProject(id: string) {
    ...
    const { data } = await supabase
      ...
      .select('content')
      .eq('project_id', id)
      .eq('type', 'initial_generation')
      .single()
    if (data?.content) setCurrentProject(data.content as GeneratedProject)
  }
